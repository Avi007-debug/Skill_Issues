...
export default function CarbonTracker() {
  ...
  const [flights, setFlights] = useState<EcoFlight[]>([]);

  useEffect(() => {
    const loadFlights = async () => {
      try {
        const result = await apiFetch<{ items: Flight[] }>("/flights/eco");
        const mapped = (result.items || []).map((f) => ({
          id: f.id,
          airline: f.airline,
          ...
          duration: f.duration,
          price: f.price,
          carbonEmission: f.co2_kg,
          isEcoFriendly: f.is_eco_friendly,
        }));
        setFlights(mapped);
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "Could not load eco flights");
      }
    };
